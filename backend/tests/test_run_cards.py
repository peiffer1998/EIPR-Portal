"""Printable run cards endpoint tests."""
...
async def test_run_cards_contains_schedule_data(app_context: dict[str, Any]) -> None:
    ...
    await _create_feeding_schedule(
        ...
        headers,
        reservation_id=reservation_id,
        scheduled_at=now.replace(hour=7),
        food="Beef & Sweet Potato",
    )
    await _create_medication_schedule(
        client,
        headers,
        reservation_id=reservation_id,
        scheduled_at=now.replace(hour=20),
    ...
    response = await client.get(
        ...
        params={"date": target_date, "location_id": location_id},
        headers=headers,
    )
    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/html")
    body = unescape(response.text)
    assert "Aspen" in body
    assert "Beef & Sweet Potato" in body
    assert "Gabapentin" in body
