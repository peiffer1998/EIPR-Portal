    ...
    if current_user.role != UserRole.SUPERADMIN:
        if account_id is not None and account_id != current_user.account_id:
            raise HTTPException(
                ...
                detail="Cannot view other accounts",
            )
...
async def create_location(
    ...
    if (
        ...
        and payload.account_id != current_user.account_id
    ):
        raise HTTPException(
...
async def read_location(
    ...
    _require_location_admin(current_user)
    account_scope = (
        None if current_user.role == UserRole.SUPERADMIN else current_user.account_id
    )
