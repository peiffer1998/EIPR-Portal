...
from app.schemas.package import ServicePackageCreate, ServicePackageRead, ServicePackageUpdate
...
async def list_packages(
    ...
    packages = await package_service.list_packages(session, account_id=current_user.account_id)
    return [ServicePackageRead.model_validate(pkg) for pkg in packages]


@router.post("", response_model=ServicePackageRead, status_code=status.HTTP_201_CREATED, summary="Create package")
...
@router.patch("/{package_id}", response_model=ServicePackageRead, summary="Update package")
async def update_package(
    ...
    updated = await package_service.update_package(session, package=package, payload=payload)
    return ServicePackageRead.model_validate(updated)


@router.delete("/{package_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete package")
