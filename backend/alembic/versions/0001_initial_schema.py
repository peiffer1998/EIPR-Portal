...
def upgrade() -> None:
    ...
    op.create_table(
        "pets",
        sa.Column("id", sa.Uuid(as_uuid=True), primary_key=True),
        sa.Column("owner_id", sa.Uuid(as_uuid=True), sa.ForeignKey("owner_profiles.id", ondelete="CASCADE"), nullable=False),
        sa.Column("home_location_id", sa.Uuid(as_uuid=True), sa.ForeignKey("locations.id", ondelete="SET NULL")),
    ...
    op.create_table(
        ...
        sa.Column("status", reservation_status_enum, nullable=False, server_default="requested"),
