...
revision = "0008_service_extensions"
down_revision = "0007_staff_invitations"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("pet_id", sa.Uuid(as_uuid=True), sa.ForeignKey("pets.id", ondelete="CASCADE"), nullable=False),
        sa.Column("reservation_type", sa.Enum(name="reservationtype"), nullable=False),
        sa.Column("desired_date", sa.Date(), nullable=False),
        sa.Column("status", waitlist_status_enum, nullable=False, server_default="pending"),
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Uuid(as_uuid=True), primary_key=True),
        sa.Column("account_id", sa.Uuid(as_uuid=True), sa.ForeignKey("accounts.id", ondelete="CASCADE"), nullable=False),
        sa.Column("owner_id", sa.Uuid(as_uuid=True), sa.ForeignKey("owner_profiles.id", ondelete="CASCADE"), nullable=True),
        sa.Column("pet_id", sa.Uuid(as_uuid=True), sa.ForeignKey("pets.id", ondelete="CASCADE"), nullable=True),
        sa.Column("uploaded_by_user_id", sa.Uuid(as_uuid=True), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True),
...
def downgrade() -> None:
    ...
    op.drop_index("ix_service_catalog_items_account_id", table_name="service_catalog_items")
