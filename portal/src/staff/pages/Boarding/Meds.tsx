...
import { getMedsBoard, updateMedsItem } from "../../lib/boardFetchers";
...
export default function MedsBoard() {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["meds-board", filters.date, filters.location_id],
    queryFn: () => getMedsBoard(filters.date, filters.location_id),
  ...
  const mutation = useMutation({
    mutationFn: ({ id, patch }: { id: string; patch: Record<string, unknown> }) => updateMedsItem(id, patch),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["meds-board", filters.date, filters.location_id] });
    },
  });
  ...
  const bulkMarkGiven = async () => {
    ...
    try {
      await Promise.all(
        ids.map((id) => updateMedsItem(id, { status: "GIVEN" }).catch(() => undefined)),
      );
  ...
  const goToPrint = () => {
    ...
    const url = `/staff/print/meds-sheet/${filters.date}?location_id=${encodeURIComponent(filters.location_id)}`;
              ...
              <tr key={row.id} className="border-t">
                ...
                <td>
                  <InlineText
                    value={row.med || ""}
                    placeholder="med"
                    onSave={(value) => mutation.mutateAsync({ id: row.id, patch: { med: value } })}
                  />
                </td>
                <td>
                  <InlineText
                    value={row.dose || ""}
                    placeholder="dose"
                    onSave={(value) => mutation.mutateAsync({ id: row.id, patch: { dose: value } })}
                  />
                </td>
                <td>
                  <InlineText
            ...
            {rows.length === 0 && !isFetching ? (
              <tr>
                <td colSpan={9} className="px-3 py-4 text-sm text-slate-500">
                  No medication items for this date and location.
                </td>
              </tr>
            ) : null}
          </tbody>
